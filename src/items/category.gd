...
@export var superClass: Item

func _init(prob: float, superClass: Item) -> void:
	self.prob = prob;
	self.superClass = superClass;


static func order(item: Item) -> int:
	for i: int in range(types.length):
		if (types[i].superClass.isInstance( item )):
			return i;



	return Integer.MAX_VALUE if item is Bag else Integer.MAX_VALUE - 1
