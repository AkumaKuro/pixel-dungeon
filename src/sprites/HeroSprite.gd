...
func updateArmor() -> void:

	TextureFilm film = new TextureFilm( tiers(), ((Hero)ch).tier(), FRAME_WIDTH, FRAME_HEIGHT );

	idle = Animation.new( 1, true );
	idle.frames( film, 0, 0, 0, 1, 0, 0, 1, 1 );

	run = Animation.new( RUN_FRAMERATE, true );
	run.frames( film, 2, 3, 4, 5, 6, 7 );

	die = Animation.new( 20, false );
	die.frames( film, 8, 9, 10, 11, 12, 11 );

	attack = Animation.new( 15, false );
